...
echo "clearpart --initlabel --drives=$disk --all" >> $file
echo "part /boot --fstype=ext4 --mkfsoptions=\"none,extent,filetype,sparse_super,flex_bg,uninit_bg,resize_inode -I 128 -N 1000\" --fsoptions=\"rw,noatime,suid,dev,exec,auto,nouser,async,stripe=4\"--recommended --label=boot --asprimary --ondrive=$drive" >> $file
...
echo "logvol / --vgname=vg_$disk --size=8000 --thin --poolname=lv_$disk --fstype=ext4 --mkfsoptions=\"-Onone,extent,ext_attr,dir_index,filetype,sparse_super,flex_bg,uninit_bg,large_file,dir_nlink,resize_inode -I 128 -i 32768\" --fsoptions=\"rw,noatime,suid,dev,exec,auto,nouser,async\" --label=root --name=lv_root" >> $file
