...
echo "logvol swap --fstype=swap --size=512 --vgname=vg_$disk --label=swap --name=lv_swap" >> $file
...
# echo "logvol / --vgname=vg_$disk --size=8000 --thin --poolname=lv_$disk --fstype=ext4 --mkfsoptions=\"-O none,extent,extra_isize,ext_attr,dir_index,filetype,sparse_super,flex_bg,uninit_bg,large_file,dir_nlink,resize_inode -I 256 -i 32768\" --label=root --name=lv_root" >> $file
echo "logvol / --vgname=vg_$disk --size=8000 --thin --poolname=lv_$disk --fstype=ext4 --label=root --name=lv_root" >> $file
