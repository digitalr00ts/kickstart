...
echo "# Generated partition scheme for $disk1" > $file
...
echo "part pv.01 --grow --ondrive=$disk2" >> $file
echo "volgroup vg_$disk2 pv.01" >> $file
if [ "$disk1" != "$disk2" ] ; then
  echo "part pv.02 --grow --ondrive=$disk1" >> $file
  echo "volgroup vg_$disk1 pv.02" >> $file
...
echo "logvol / --vgname=vg_$disk2 --name=lv_root --fstype=ext4 --mkfsoptions=\"-O none,extent,extra_isize,ext_attr,dir_index,filetype,sparse_super,flex_bg,uninit_bg,large_file,dir_nlink,resize_inode -I 256 -i 32768\" --fsoptions=\"rw,noatime,suid,dev,exec,auto,nouser,async\" --size=8000 --thin --poolname=lv_$disk1" >> $file
